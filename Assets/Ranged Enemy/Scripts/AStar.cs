    ...
    private class Pathfinding // the class used to find the path
    {
        public static void aStar(Vector3 start, Vector3 target, ref Grid g, ref int targetIndex, ref List<Node> path, ref bool pathFound, HeuristicMethod Heuristic)
        {
            ...
            while(openSet.Count > 0) // while the open set is not empty 
            {
                ...
                {
                    if (node == endNode) //if the current node is the last node
                    {
                        currentNode = node;
                        pathFound = true;
                        TracePath(startingNode, endNode, ref path, ref pathFound);
                        return;
                    }
                    CalculateHCost(node, endNode, Heuristic); //calculates the heuristic cost using a mehtod in grid
                    float cost = node.Cost; //calculates the heuristic cost using a mehtod in grid
                ...
                foreach(Node n in g.GetNeighbours(currentNode)) //for each neighbour of the current node
                {
                    ...
                    float gVal = currentNode.g + g.GetDist(currentNode, n); //calculates the g value by getting the distance between the current node and the neighbour
